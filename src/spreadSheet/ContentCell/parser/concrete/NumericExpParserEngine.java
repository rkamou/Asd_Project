...
public class NumericExpParserEngine implements CellParserEngine{
	...
	public Contents parser(String expression) {
		...
		if(isDouble(expression)) return new Numeric((float)Double.parseDouble(expression));

		if(isNumericFormulas(expression))
			if(isConstantInExpression(expression)) return sortList(this.tContents(listContents));
		
		if(isReferenceInExpression(expression)) return sortList(this.tContents(listContents));
		return null;
	}
...
//					System.out.println("Substraction Occurs index " + i + " Index " + j);
	...
	public boolean isReferenceInExpression(String expression) {
		...
		while (i < expression.length()) {
			if((expression.charAt(i) + "").equals("[")){ i++;
				int row = 0,col = 0; term = ""; boolean end = false;
				while(i < expression.length() && end ==false) {
					if((expression.charAt(i) + "").matches("\\d"))	term += (expression.charAt(i) + "");
					if((expression.charAt(i) + "").equals(","))
						if(isDouble(term)) { row = Integer.parseInt(term); term = "";}else return false;
					if((expression.charAt(i) + "").equals("]")) {
						if(isDouble(term)) { col = Integer.parseInt(term); term = "";}else return false;
						end = true;
					}
					i++; 
